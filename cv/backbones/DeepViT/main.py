...
import torch
from cv.backbones import DeepViT
...
if __name__ == "__main__":
    try:
        ...
        if cfg.ASYNC_TRAINING:
            ...
            mp.spawn(Train.async_train, args=("DeepViT", world_size), nprocs=world_size, join=True)
        else:
            ...
            model = DeepViT(**Global.CFG.DeepViT.PARAMS)
            ...
            Global.LOGGER.info(f"Instantiating Optimizer: {cfg.DeepViT.OPTIMIZER.NAME}")
            optimizer = getattr(torch.optim, cfg.DeepViT.OPTIMIZER.NAME)(model.parameters(), **cfg.DeepViT.OPTIMIZER.PARAMS)
            Global.LOGGER.info(f"Optimizer instantiated")

            train_dataset = ClassificationDataset("train", transforms=cfg.DeepViT.TRANSFORMS.TRAIN, debug=Global.CFG.DEBUG)
            val_dataset = ClassificationDataset("val", transforms=cfg.DeepViT.TRANSFORMS.VAL, debug=Global.CFG.DEBUG // 2)
            ...
            data_loaders["train"] = DataLoader(
                ...
                **cfg.DeepViT.DATALOADER_TRAIN_PARAMS
            )
            data_loaders["val"] = DataLoader(
                dataset=val_dataset, collate_fn=val_dataset.collate_fn,
                **cfg.DeepViT.DATALOADER_VAL_PARAMS
            )

            Global.LOGGER.info(f"Data loaders instantiated")

            Global.LOGGER.info(f"Instantiating Learning Rate Scheduler: {cfg.DeepViT.LR_SCHEDULER.NAME}")

            lr_scheduler = Train.get_lr_scheduler(
                scheduler_name=cfg.DeepViT.LR_SCHEDULER.NAME,
                optimizer=optimizer,
                scheduler_params=cfg.DeepViT.LR_SCHEDULER.PARAMS
            )
            ...
            Global.LOGGER.info(f"Instantiating Loss Function: {cfg.DeepViT.LOSS.NAME}")
            loss_function = getattr(torch.nn, cfg.DeepViT.LOSS.NAME)(**cfg.DeepViT.LOSS.PARAMS)
            ...
            train = Train(
                ...
                tb_writer=tb_writer,
                gradient_clipping=Global.CFG.DeepViT.UTILITIES.GRADIENT_CLIPPING,
