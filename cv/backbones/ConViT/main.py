...
import torch
from cv.backbones import ConViT
...
if __name__ == "__main__":
    try:
        ...
        if cfg.ASYNC_TRAINING:
            ...
            mp.spawn(Train.async_train, args=("ConViT", world_size), nprocs=world_size, join=True)
            ...
            model = ConViT(**Global.CFG.ConViT.PARAMS)
            ...
            Global.LOGGER.info(f"Instantiating Optimizer: {cfg.ConViT.OPTIMIZER.NAME}")
            optimizer = getattr(torch.optim, cfg.ConViT.OPTIMIZER.NAME)(model.parameters(), **cfg.ConViT.OPTIMIZER.PARAMS)
            Global.LOGGER.info(f"Optimizer instantiated")

            train_dataset = ClassificationDataset("train", transforms=cfg.ConViT.TRANSFORMS.TRAIN, debug=Global.CFG.DEBUG)
            val_dataset = ClassificationDataset("val", transforms=cfg.ConViT.TRANSFORMS.VAL, debug=Global.CFG.DEBUG // 2)
            ...
            data_loaders["train"] = DataLoader(
                ...
                **cfg.ConViT.DATALOADER_TRAIN_PARAMS
            )
            data_loaders["val"] = DataLoader(
                dataset=val_dataset, collate_fn=val_dataset.collate_fn,
                **cfg.ConViT.DATALOADER_VAL_PARAMS
            )

            Global.LOGGER.info(f"Data loaders instantiated")

            Global.LOGGER.info(f"Instantiating Learning Rate Scheduler: {cfg.ConViT.LR_SCHEDULER.NAME}")

            lr_scheduler = Train.get_lr_scheduler(
                scheduler_name=cfg.ConViT.LR_SCHEDULER.NAME,
                optimizer=optimizer,
                scheduler_params=cfg.ConViT.LR_SCHEDULER.PARAMS
            )
            ...
            Global.LOGGER.info(f"Instantiating Loss Function: {cfg.ConViT.LOSS.NAME}")
            loss_function = getattr(torch.nn, cfg.ConViT.LOSS.NAME)(**cfg.ConViT.LOSS.PARAMS)
            ...
            train = Train(
                ...
                tb_writer=tb_writer,
                gradient_clipping=Global.CFG.ConViT.UTILITIES.GRADIENT_CLIPPING,
