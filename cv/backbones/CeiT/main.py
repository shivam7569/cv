...
if __name__ == "__main__":
    try:
        ...
        if cfg.ASYNC_TRAINING:
            ...
            mp.spawn(Train.async_train, args=("CeiT", world_size), nprocs=world_size, join=True)
        else:
            ...
            model = CeiT(**Global.CFG.CeiT.PARAMS)
            ...
            Global.LOGGER.info(f"Instantiating Optimizer: {cfg.CeiT.OPTIMIZER.NAME}")
            optimizer = getattr(torch.optim, cfg.CeiT.OPTIMIZER.NAME)(model.parameters(), **cfg.CeiT.OPTIMIZER.PARAMS)
            Global.LOGGER.info(f"Optimizer instantiated")

            train_dataset = ClassificationDataset("train", transforms=cfg.CeiT.TRANSFORMS.TRAIN, debug=Global.CFG.DEBUG)
            val_dataset = ClassificationDataset("val", transforms=cfg.CeiT.TRANSFORMS.VAL, debug=Global.CFG.DEBUG // 2)
            ...
            data_loaders["train"] = DataLoader(
                ...
                **cfg.CeiT.DATALOADER_TRAIN_PARAMS
            )
            data_loaders["val"] = DataLoader(
                dataset=val_dataset, collate_fn=val_dataset.collate_fn,
                **cfg.CeiT.DATALOADER_VAL_PARAMS
            )

            Global.LOGGER.info(f"Data loaders instantiated")

            Global.LOGGER.info(f"Instantiating Learning Rate Scheduler: {cfg.CeiT.LR_SCHEDULER.NAME}")

            lr_scheduler = Train.get_lr_scheduler(
                scheduler_name=cfg.CeiT.LR_SCHEDULER.NAME,
                optimizer=optimizer,
                scheduler_params=cfg.CeiT.LR_SCHEDULER.PARAMS
            )
            ...
            Global.LOGGER.info(f"Instantiating Loss Function: {cfg.CeiT.LOSS.NAME}")
            loss_function = getattr(torch.nn, cfg.CeiT.LOSS.NAME)(**cfg.CeiT.LOSS.PARAMS)
            ...
            train = Train(
                ...
                tb_writer=tb_writer,
                gradient_clipping=Global.CFG.CeiT.UTILITIES.GRADIENT_CLIPPING,
