...
from cv import semseg
from cv.src import losses
from cv.src import custom_lrs
from cv.src import optimizers
from cv.utils import MetaWrapper
from cv.utils.os_utils import check_dir
from cv.phases.segmentation import Eval
...
from cv.configs.config import CfgNode, setup_config
...
class Train(metaclass=MetaWrapper):
    ...
    def __class_repr__(cls):
        return "Training Class for all the segmentation models"
        ...
        for idx, batch in enumerate(data_iterator):
            ...
            if Global.CFG.SAVE_FIRST_SAMPLE:
                if (self.tb_writer is not None) and (not self.sample_batch_log) and (epoch == 0):
                    self.tb_writer.write("image")(image=SegmentationDataset._vizualizeBatch(batch), epoch=epoch+1, tag="Training Sample")
