...
if __name__ == "__main__":
    try:
        ...
        if cfg.ASYNC_TRAINING:
            ...
            mp.spawn(Train.async_train, args=("TNT", world_size), nprocs=world_size, join=True)
            ...
            model = TNT(**Global.CFG.TNT.PARAMS)
            ...
            Global.LOGGER.info(f"Instantiating Optimizer: {cfg.TNT.OPTIMIZER.NAME}")
            optimizer = getattr(torch.optim, cfg.TNT.OPTIMIZER.NAME)(model.parameters(), **cfg.TNT.OPTIMIZER.PARAMS)
            Global.LOGGER.info(f"Optimizer instantiated")

            train_dataset = ClassificationDataset("train", transforms=cfg.TNT.TRANSFORMS.TRAIN, debug=Global.CFG.DEBUG)
            val_dataset = ClassificationDataset("val", transforms=cfg.TNT.TRANSFORMS.VAL, debug=Global.CFG.DEBUG // 2)
            ...
            data_loaders["train"] = DataLoader(
                ...
                **cfg.TNT.DATALOADER_TRAIN_PARAMS
            )
            data_loaders["val"] = DataLoader(
                dataset=val_dataset, collate_fn=val_dataset.collate_fn,
                **cfg.TNT.DATALOADER_VAL_PARAMS
            )

            Global.LOGGER.info(f"Data loaders instantiated")

            Global.LOGGER.info(f"Instantiating Learning Rate Scheduler: {cfg.TNT.LR_SCHEDULER.NAME}")

            lr_scheduler = Train.get_lr_scheduler(
                scheduler_name=cfg.TNT.LR_SCHEDULER.NAME,
                optimizer=optimizer,
                scheduler_params=cfg.TNT.LR_SCHEDULER.PARAMS
            )
            ...
            Global.LOGGER.info(f"Instantiating Loss Function: {cfg.TNT.LOSS.NAME}")
            loss_function = getattr(torch.nn, cfg.TNT.LOSS.NAME)(**cfg.TNT.LOSS.PARAMS)
            ...
            train = Train(
                ...
                tb_writer=tb_writer,
                gradient_clipping=Global.CFG.TNT.UTILITIES.GRADIENT_CLIPPING,
