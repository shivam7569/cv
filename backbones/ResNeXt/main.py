...
if __name__ == "__main__":
    try:
        ...
        model = ResNeXt(num_classes=1000)
        Global.LOGGER.info(f"{cfg.LOGGING.NAME} Architecture instantiated")

        Global.LOGGER.info(f"Instantiating Optimizer: {cfg.ResNeXt.OPTIMIZER.NAME}")
        optimizer = getattr(torch.optim, cfg.ResNeXt.OPTIMIZER.NAME)(model.parameters(), **cfg.ResNeXt.OPTIMIZER.PARAMS)
        Global.LOGGER.info(f"Optimizer instantiated")

        train_dataset = ClassificationDataset("train", transforms=cfg.ResNeXt.TRANSFORMS.TRAIN, debug=None)
        val_dataset = ClassificationDataset("val", transforms=cfg.ResNeXt.TRANSFORMS.VAL, debug=None)
        ...
        data_loaders["train"] = DataLoader(
            ...
            **cfg.ResNeXt.DATALOADER_TRAIN_PARAMS
        )
        data_loaders["val"] = DataLoader(
            dataset=val_dataset,
            **cfg.ResNeXt.DATALOADER_VAL_PARAMS
        )

        Global.LOGGER.info(f"Data loaders instantiated")

        Global.LOGGER.info(f"Instantiating Learning Rate Scheduler: {cfg.ResNeXt.LR_SCHEDULER.NAME}")

        if cfg.ResNeXt.LR_SCHEDULER.NAME == "LambdaLR":
            lr_lambda = lambda epoch: 0.96 * epoch
            lr_scheduler = getattr(
                torch.optim.lr_scheduler, cfg.ResNeXt.LR_SCHEDULER.NAME
                )(optimizer, lr_lambda=lr_lambda, **cfg.ResNeXt.LR_SCHEDULER.PARAMS)
        else: 
            lr_scheduler = getattr(
                torch.optim.lr_scheduler, cfg.ResNeXt.LR_SCHEDULER.NAME
                )(optimizer, **cfg.ResNeXt.LR_SCHEDULER.PARAMS)
        Global.LOGGER.info(f"Learning Rate Scheduler instantiated")

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        
        Global.LOGGER.info(f"Instantiating Loss Function: {cfg.ResNeXt.LOSS.NAME}")
        loss_function = getattr(torch.nn, cfg.ResNeXt.LOSS.NAME)(**cfg.ResNeXt.LOSS.PARAMS)
