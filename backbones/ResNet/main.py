...
if __name__ == "__main__":
    try:
        ...
        if cfg.ASYNC_TRAINING:
            ...
            Global.LOGGER.info(f"Instantiating Optimizer: {cfg.ResNet.OPTIMIZER.NAME}")
            optimizer = getattr(torch.optim, cfg.ResNet.OPTIMIZER.NAME)(model.parameters(), **cfg.ResNet.OPTIMIZER.PARAMS)
            Global.LOGGER.info(f"Optimizer instantiated")

            train_dataset = ClassificationDataset("train", transforms=cfg.ResNet.TRANSFORMS.TRAIN, debug=None)
            val_dataset = ClassificationDataset("val", transforms=cfg.ResNet.TRANSFORMS.VAL, debug=None)
            ...
            data_loaders["train"] = DataLoader(
                ...
                **cfg.ResNet.DATALOADER_TRAIN_PARAMS
            )
            data_loaders["val"] = DataLoader(
                dataset=val_dataset,
                **cfg.ResNet.DATALOADER_VAL_PARAMS
            )

            Global.LOGGER.info(f"Data loaders instantiated")

            Global.LOGGER.info(f"Instantiating Learning Rate Scheduler: {cfg.ResNet.LR_SCHEDULER.NAME}")

            if cfg.ResNet.LR_SCHEDULER.NAME == "LambdaLR":
                ...
                lr_scheduler = getattr(
                    torch.optim.lr_scheduler, cfg.ResNet.LR_SCHEDULER.NAME
                    )(optimizer, **cfg.ResNet.LR_SCHEDULER.PARAMS)
            Global.LOGGER.info(f"Learning Rate Scheduler instantiated")

            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            
            Global.LOGGER.info(f"Instantiating Loss Function: {cfg.ResNet.LOSS.NAME}")
            loss_function = getattr(torch.nn, cfg.ResNet.LOSS.NAME)(**cfg.ResNet.LOSS.PARAMS)
