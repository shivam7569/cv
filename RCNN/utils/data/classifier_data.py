...
def parse_annotation_jpeg(coco, imgID, directory, ss):
    ...
    for data in anns:
        ...
        if (x2 - x1) > 16 and (y2 - y1) > 16:
            ...
            positive_list["proposal_coord"].append(
                [str(x1), str(y1), str(x2), str(
                    y2), str(data["category_id"])]
            )
    ...
    for i in range(rects.shape[0]):
        ...
        if (0.0 < iou_score <= 0.3) and (rect_size > maximum_bndbox_size / 5):
...
def process_data(args):
    ...
    dst_root_dir = os.path.join(Global.CLASSIFIER_DATA_DIR, directory)
